...
/-- Compact fixed points below `x`: the isolated points `a(e n)` contained in `x`. -/
...
theorem retractBasis_eq_compactFixedBelow {a : Pomega} (ha : IsClosure a)
  ...
  · rintro ⟨⟨n, hkn⟩, hkx⟩
    refine ⟨n, ?_, hkn⟩
    have hsub : funOf a (e n) ⊆ (x : Pomega) := by
      rwa [hkn] at hkx
    exact (closure_expansive ha (e n)).trans hsub
...
theorem compactFixedBelow_directedSubset {a : Pomega} (ha : IsClosure a)
    (x : Fixpoints (funOf a)) :
    IsDirectedSubset (compactFixedBelow a x) := by
  rw [← retractBasis_eq_compactFixedBelow ha x]
  exact retractBasis_directedSubset ha.2 x

theorem compactFixedBelow_sUnion {a : Pomega} (ha : IsClosure a)
    (x : Fixpoints (funOf a)) :
    (x : Pomega) = ⋃₀ (Subtype.val '' compactFixedBelow a x) := by
  rw [← retractBasis_eq_compactFixedBelow ha x]
  exact retractBasis_sUnion ha.2 x

theorem directed_finite_cover_subset {f : Pomega → Pomega}
    {A : Set (Fixpoints f)} (hA : IsDirectedSubset A) {t : Pomega}
    (ht : t.Finite) (hsub : t ⊆ ⋃₀ (Subtype.val '' A)) :
    ∃ z ∈ A, t ⊆ z := by
  have hA' : IsDirectedSet A := by
    refine ⟨hA.1, ?_⟩
    intro x hx y hy
    obtain ⟨z, hz, hxz, hyz⟩ := hA.2 hx hy
    exact ⟨z, hz, hxz, hyz⟩
  simpa [Fixpoints.vals] using directed_finite_cover hA' ht (by
    simpa [Fixpoints.vals] using hsub)

theorem closureCompact_isCompactSubset {a : Pomega} (ha : IsClosure a)
    (n : ℕ) : IsCompactSubset (closureCompact a ha n) := by
  intro s hs hsub
  have hen : e n ⊆ ⋃₀ (Subtype.val '' s) :=
    (closure_expansive ha (e n)).trans hsub
  obtain ⟨z, hz, hnz⟩ := directed_finite_cover_subset hs (e_finite n) hen
  refine ⟨z, hz, ?_⟩
  have := isScottContinuous_monotone (funOf_isScottContinuous a) hnz
  rwa [z.property] at this

theorem compact_fixed_iff_subset {a : Pomega} (ha : IsClosure a)
    (x : Fixpoints (funOf a)) :
    IsCompactSubset x ↔ ∃ n, x = closureCompact a ha n := by
  constructor
  · intro hx
    obtain ⟨y, hy, hxy⟩ :=
      hx (compactFixedBelow a x) (compactFixedBelow_directedSubset ha x)
        (le_of_eq (compactFixedBelow_sUnion ha x))
    obtain ⟨⟨n, hn⟩, hyx⟩ := hy
    have hyx' : (y : Pomega) ⊆ x := hyx
    have : y = x := Subtype.ext (subset_antisymm hyx' hxy)
    refine ⟨n, this.symm.trans (Subtype.ext hn)⟩
  · rintro ⟨n, rfl⟩
    exact closureCompact_isCompactSubset ha n
...
theorem theorem_5_1 {a : Pomega} (ha : IsClosure a) :
    ...
    ∀ x : Fixpoints (funOf a),
      IsDirectedSubset (compactFixedBelow a x) ∧
        (x : Pomega) = ⋃₀ (Subtype.val '' compactFixedBelow a x) :=
  ⟨compact_fixed_iff_subset ha, fun x =>
    ⟨compactFixedBelow_directedSubset ha x, compactFixedBelow_sUnion ha x⟩⟩
...
def closureRangeCountablyAlgebraic (a : Pomega) (ha : IsClosure a) :
  ...
  density := fun x => by
    have hsets : {k | IsCompact k ∧ k ≤ x} = compactFixedBelow a x := by
      ext k
      constructor
      · intro ⟨hk, hkx⟩
        obtain ⟨n, hn⟩ := (compact_fixed_iff ha k).mp hk
        exact ⟨⟨n, congrArg Subtype.val hn⟩, hkx⟩
      · intro ⟨⟨n, hn⟩, hkx⟩
        exact ⟨(compact_fixed_iff ha k).mpr ⟨n, Subtype.ext hn⟩, hkx⟩
    rw [hsets]
    exact (fixed_eq_sSup_compacts ha x).2
    ...
    IsScottContinuous (representClosure D) ∧
      ...
      ∃ φ : α → Fixpoints (funOf (representClosureCode D)),
        Function.Bijective φ ∧
          ∀ x y, x ≤ y ↔ (φ x : Pomega) ⊆ φ y :=
  ⟨representClosure_isScottContinuous D,
    representClosureCode_isClosure D,
    ⟨fun x => representationOrderIso D x,
      (representationOrderIso D).toEquiv.bijective,
      fun x y => by
        constructor
        · intro hxy
          exact (representationOrderIso D).map_rel_iff.mpr hxy
        · intro hsub
          exact (representationOrderIso D).map_rel_iff.mp hsub⟩⟩
