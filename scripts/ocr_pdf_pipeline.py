...
Usage (from repo root):
  bash scripts/ocr_pdf_pipeline.sh Data_Types_as_Lattices.pdf
  bash scripts/ocr_pdf_pipeline.sh Data_Types_as_Lattices.pdf --pages 1-3
  bash scripts/ocr_pdf_pipeline.sh Data_Types_as_Lattices.pdf --png-only
  bash scripts/ocr_pdf_pipeline.sh Data_Types_as_Lattices.pdf --merge-only
  bash scripts/ocr_pdf_pipeline.sh Data_Types_as_Lattices.pdf --status
"""
...
Use LaTeX for math: $D_\\infty$, $\\lambda$, $\\sqsubseteq$, $\\bot$, etc.
...
- Omit standalone printed page numbers (e.g. lines that are only "522" or "523").
...
# SIAM J. Comput. 5 (1976) reprint pagination.
SIAM_PAGE = re.compile(r"^\s*(5[2-7][0-9]|58[0-7])\s*$")
...
def strip_book_page_numbers(text: str) -> str:
    """Remove SIAM printed page numbers accidentally transcribed as standalone lines."""
    return "\n".join(
        line for line in text.splitlines() if not SIAM_PAGE.match(line)
    ).strip() + "\n"
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        nargs="?",
        default=Path("Data_Types_as_Lattices.pdf"),
        help="PDF name or path (default: Data_Types_as_Lattices.pdf under sources/)",
    )
