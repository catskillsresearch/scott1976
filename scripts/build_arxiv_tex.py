...
GITHUB_URL = r"https://github.com/catskillsresearch/scott1976"
PALOMAR_URL = (
    r"https://palomar-registry.org/entry?id=PALOMAR-2026-08-25-000004&version=1"
)
...
def extract_lean_titles(text: str) -> dict[str, str]:
    ...
    for idx, pos in enumerate(lean_starts):
        ...
        for line in reversed(prefix.splitlines()[-4:]):
            m = re.match(r"^###\s+(Scott1976(?:\.lean|/[^\s{]+))", line.strip())
            if m:
                module = m.group(1)
                break
